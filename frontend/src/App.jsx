...
export default function App() {
  ...
  async function handleUpload(e) {
    e.preventDefault();
    if (!file) return;
    setStatus("Uploading and generating font... (this can take 10-60s)");
    ...
    try {
      ...
      setStatus("Font ready. Scroll to preview & download.");
    } catch (err) {
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", padding: 24 }}>
      ...
      <form onSubmit={handleUpload}>
        ...
        <button style={{ marginTop: 12 }} type="submit">Upload & Generate</button>
      </form>
      ...
      {fontUrl && (
        <>
          <h2>Preview</h2>
          <textarea
            style={{ fontFamily: "UserHandFont, sans-serif", fontSize: 20, width: "100%", height: 120 }}
            value={previewText}
            onChange={e => setPreviewText(e.target.value)}
          />
          <div style={{ marginTop: 12 }}>
            <a href={fontUrl} download={`${fontName}.ttf`}>Download .ttf</a>
          </div>
        </>
      )}
    </div>
